...
**悲观锁：**
 ...
 悲观锁可以实现对于数据的串行化执行，比如**synchronized**，和lock都是悲观锁的代表，同时，悲观锁中又可以再细分为公平锁，非公平锁，可重入锁，等等
...
这段代码的目的是设置一个用于解锁操作的 Redis 脚本（`unlock.lua`）。具体来说，它涉及到以下几个关键点：

1. **`private static final DefaultRedisScript<Long> UNLOCK_SCRIPT;`**
   这一行声明了一个 `UNLOCK_SCRIPT` 变量，这是一个类型为 `DefaultRedisScript<Long>` 的静态常量。`DefaultRedisScript` 是 Spring Data Redis 提供的一个类，用于封装 Redis 脚本。这里指定了脚本执行后返回的结果类型为 `Long`。
2. **`static { ... }`**
   这是一个静态代码块，会在类加载时执行一次。它用于初始化 `UNLOCK_SCRIPT` 常量。
3. **`UNLOCK_SCRIPT.setLocation(new ClassPathResource("unlock.lua"));`**
   这一行设置了 Redis 脚本的路径。`ClassPathResource` 表示脚本文件 `unlock.lua` 位于类路径下，通常是项目的 `resources` 目录。
4. **`UNLOCK_SCRIPT.setResultType(Long.class);`**
   这一行指定了 Redis 脚本执行后的返回类型为 `Long`。假设脚本执行的结果会返回一个长整型数值，可能是解锁成功与否的标识符或其他的返回值。
...
![{7B48F28C-F520-491F-A42E-AFF1915C53B2}](assets/{7B48F28C-F520-491F-A42E-AFF1915C53B2}.png)
...
当以上lua表达式   执行完毕后，剩下的就是根据步骤3,4来执行我们接下来的任务了
...
计算方式：开始的索引=（page-1）*size

在t1时刻，开始的索引就是0，10的索引是0，所以从10开始
...
public Result signCount() {
    ...
    while (true) {
        ...
        // 把数字右移一位，抛弃最后一个bit位，继续下一个bit位
            //这里以一定要选择无符号右移，因为无符号右移默认最高位补0，这样如果前面的位全是1，那么到了第一次补0的位就会停止程序
            //如果有符号右移，那么最高位是1的话，前面会补1，如果中间全是1，就不会出现(num & 1) == 0的情况，陷入死循环
        num >>>= 1;
    }
